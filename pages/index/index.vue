<template>
	<Container style="padding-top: 100rpx;" v-model="current" tabbar :is-back="false"
	 :style="[{backgroundColor:'var(--background-color-1)'}]">
		 <view slot="action" v-if="(current==='home'||current==='shopping-cart')&&userType+''!=='1'&&pickerData.length>0">
			<picker @change="PickerChange" :value="index" :range="pickerData">
				<view class="picker">
					<text class="text" style="padding-right:16rpx">
						{{index>-1?pickerData[index]:'选择客户'}}
					</text>
		...
		<template v-if="current==='home'">
			...
			<scroll-view scroll-y class="indexes" :scroll-into-view="'indexes-'+ listCurID" :style="[{height:'calc(100vh - '+ CustomBar + 'px - 50px)',backgroundColor:'var(--background-color-1)'}]"
			 :scroll-with-animation="true" :enable-back-to-top="true">
				<block v-for="(item,index) in list" :key="index">
					<view :class="'indexItem-' + item.name" :id="'indexes-' + item.name" :data-index="item.name" v-if="item.data&&item.data.length>0">
			...
			<view class="index-bar" :style="[{height:'calc(100vh - ' + CustomBar + 'px - 50px)'}]">
				<view class="index-bar-box" @touchstart="tStart" @touchend="tEnd" @touchmove.stop="tMove">
					<view class="index-bar-item" v-for="(item,index) in list3" :key="index" :id="index" @touchstart="getCur" @touchend="setCur">
						{{item.name}}</view>
				</view>
			</view>
			<!--选择显示-->
		...
		<Order v-else-if="current==='order'||current==='track'" :mode="current"></Order>
	</Container>
</template>
		...
		methods: {
			...
			search(){
				...
				if(that.searchContent){
					const bon=that.searchContent.match(/[a-zA-Z]/g);
					if(bon){
						console.log('bon[0].toLocaleUpperCase()',bon[0].toLocaleUpperCase())
						that.list=data.filter(item=>{
							return item.name===bon[0].toLocaleUpperCase();
						})
					}else{
						that.list=data.map(item=>{
							let arr=item.data.filter(item2=>{
							   return item2.name.indexOf(that.searchContent)!==-1
						  })
							return {
								name: item.name,
								data:arr
					...
					that.list= this.list2.slice(0,3);
					that.count=3;
					that.handleList();
				}
			},
			nameChange(e){
				...
				console.log('this.searchContent',this.searchContent,e.detail.value)
				this.search();
			},
			handleList(){
				const that=this;
			...
			debounce(fn, wait) {
			    return (val,index,sub) => {
			        if (this.timeout !== null) {
			            clearTimeout(this.timeout)
			        }
			        this.timeout = setTimeout(() => fn(val,index,sub), wait)
			    }
			},
			debounceCart(val,index,sub){
				this.$loading.open();
				this.debounce(this.addShoppingCart,400)(val,index,sub);
			},
			addShoppingCart(val,index,sub) {
				console.log(2222,val,index,sub)
				const that = this;
				let data = that.$store.getLoginInfo();
				if(!(data&&data.userId)){
					that.$loading.close()
				...
				cart({
					...
					if(res.resCode===0){
						...
						this.$set(this.list[index].data,sub,{
							...obj,
							active: true
						});
			...
			PickerChange(e) {
				...
				this.$store.setPharmacy(this.picker[this.index])
			},
			//获取文字信息
			getCur(e) {
				this.hidden = false;
