<template>
  ...
  >
    ...
    <Track v-else-if="current==='track'" />
		<Login ref="Login" @change="loginChange"></Login>
  </Container>
</template>
			...
			wx.getSetting({
				success(res) {
					...
					if (res.authSetting['scope.userInfo']) {
						that.authorizeBtn = false;
					} else {
						that.authorizeBtn = true;
					}
				}
			})
		},
		loginChange(res){
			console.log('loginChange',res)
			login({
				"code": res.code,
				"rawData": res.rawData,
				...
				"iv": res.iv,
				"phoneEncryptedData": res.phoneEncryptedData,
				"phoneIv": res.phoneIv,
			}).then((val) => {
